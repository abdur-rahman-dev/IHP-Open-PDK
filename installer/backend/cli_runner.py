...
from .checker import build_install_plan, validate_github_source, validate_local_source
...
def run_nogui_install(args: Namespace, pdk_root: str, stdout=None, stderr=None) -> int:
    ...
    override_row = _get_install_destination_row(plan)
    if (
        override_row
        and override_row.requires_confirmation
        and override_row.reason_code == "install_destination_override"
        and not getattr(args, "allow_override", False)
    ):
        destination = override_row.expected_value or override_row.current_value or config.get_target_pdk_dir()
        print(
            f"ERROR: Installation would override existing destination: {destination}",
            file=stderr,
            flush=True,
        )
